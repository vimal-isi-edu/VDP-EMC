...
def arg_parse():
    ...
    parser.add_argument('--align_weight', type=float, help='Alignment loss weight', default=1.0)
...
def main():
    ...
    cfg.RPIN.ALIGN_WEIGHT = args.align_weight
    cfg.RPIN.ROI_POOL_SIZE = args.pool_size
    cfg.freeze()
    ...
    # ---- setup wandb
    dataset_name = cfg.DATASET_ABS
    wandb_mode = 'online' if args.wandb else 'disabled'
    wandb_job_name = '{}'.format(args.output)
    wandb_logger = wandb.init(project="placeholder", entity="placeholder", config=args, name=wandb_job_name, mode=wandb_mode)
