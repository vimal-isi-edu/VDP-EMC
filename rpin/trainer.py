...
class Trainer(object):
    def __init__(self, device, train_loader, val_loader, model, optim,
                max_iters, num_gpus, logger, output_dir, wandb_logger):
        # misc
        self.device = device
    ...
    def train_epoch(self):
        for batch_idx, (data, data_t, rois, gt_boxes, gt_masks, valid, g_idx, seq_l, env_mask, gt_rois) in enumerate(self.train_loader):
            ...
            if C.RPIN.USE_ALIGN:
                gt_rois = xyxy_to_rois(gt_rois, batch=data.shape[0], time_step=gt_rois.shape[1], num_devices=self.num_gpus)
            else:
                gt_rois = None
            self.optim.zero_grad()

            outputs = self.model(data, rois, gt_rois, num_rollouts=self.ptrain_size, g_idx=g_idx, x_t=data_t, phase='train')
            labels = {
            ...
            # print_msg += ' | {:.3f}'.format(self.losses['align'] / self.loss_cnt)
            ...
            # Logging to Wandb
            wandb_logging_message = {'train/mean_loss': mean_loss}
            for name in self.loss_name:
                wandb_logging_message.update({'train/{}'.format(name): self.losses[name] * 1e3 / self.loss_cnt})
            if C.RPIN.SEQ_CLS_LOSS_WEIGHT:
                wandb_logging_message.update({'train/fg_correct_rate': self.fg_correct / self.fg_num,
                                            'train/bg_correct_rate': self.bg_correct / self.bg_num})
            self.wandb_logger.log(wandb_logging_message)
    ...
    def val(self):
        ...
        for batch_idx, (data, _, rois, gt_boxes, gt_masks, valid, g_idx, seq_l, env_mask, gt_rois) in enumerate(self.val_loader):
        ...
        #Logging val result to wandb
        wandb_logging_message = {'test/epoch': self.epochs,
                                'test/iter': self.iterations // 1000,
                                'test/mean_loss': mean_loss}
        for name in self.loss_name:
            wandb_logging_message.update({'test/{}'.format(name): self.losses[name] * 1e3 / self.loss_cnt})
        if C.RPIN.SEQ_CLS_LOSS_WEIGHT:
            wandb_logging_message.update({'test/fg_correct_rate': self.fg_correct / (self.fg_num + 1e-9),
                                        'test/bg_correct_rate': self.bg_correct / (self.bg_num + 1e-9)})
        self.wandb_logger.log(wandb_logging_message)
    ...
    def loss(self, outputs, labels, phase):
        ...
        align_loss = 0
        if C.RPIN.USE_ALIGN and phase == 'train':
            pred_state = outputs['pred_state']
            gt_state = outputs['gt_state']
            align_loss = self.mseloss(pred_state, gt_state).mean((2,3,4,5))
            self.losses['align'] += align_loss.mean(1).sum(0).item()
            align_loss = align_loss.mean(0)
        ...
        align_loss  = ((align_loss * tau) / tau.sum(axis=0, keepdims=True)).sum()
        loss = loss + mask_loss + kl_loss + seq_loss + C.RPIN.ALIGN_WEIGHT * align_loss
