...
class Phys(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        env_mask = self._parse_mask(video_name, vid_idx, img_idx, boxes, data)
        ...
        if random.random() > 0.5 and self.split == 'train' and C.RPIN.HORIZONTAL_FLIP:
            ...
            env_mask = np.ascontiguousarray(env_mask[..., ::-1])
        ...
        gt_rois = boxes[self.input_size:].copy()
        # gt boxes
        ...
        gt_rois = torch.from_numpy(gt_rois.astype(np.float32))
        ...
        env_mask = torch.from_numpy(env_mask.astype(np.float32))

        return data, data_t, rois, gt_boxes, gt_masks, valid, g_idx, labels, env_mask, gt_rois
