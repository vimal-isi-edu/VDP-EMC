...
class Net(nn.Module):
    ...
    def forward(self, x, rois, gt_rois, num_rollouts=10, g_idx=None, x_t=None, phase='train'):
        ...
        # of shape (b, t, o, dim)
        if phase == 'train' and C.RPIN.USE_ALIGN:
            x, gt_states = self.extract_object_feature(x, rois, gt_rois)
        else:
            x, _ = self.extract_object_feature(x, rois)
            gt_states = None
        ...
        pred_state = torch.stack(state_list_buffer[4:]).permute(1, 0, 2, 3, 4, 5)
    ...
    def extract_object_feature(self, x, rois, gt_rois=None):
        ...
        if gt_rois is not None:
            hg_output = x.reshape((batch_size, time_step, ) + x.shape[1:])[:, 0, :, :, :].unsqueeze(1)
            hg_output = hg_output.repeat(1, gt_rois.shape[1], 1, 1, 1).reshape((batch_size * gt_rois.shape[1], ) + hg_output.shape[2:])
            gt_roi_pool = self.roi_align(hg_output, gt_rois.reshape(-1, 5))
            gt_roi_pool = self.roi2state(gt_roi_pool)
            gt_states = gt_roi_pool.reshape((batch_size, gt_rois.shape[1], self.num_objs) + (gt_roi_pool.shape[-3:]))
