...
def bounce_n(T=128, n=2, r=None, m=None, ball_move_size=None):
    if m is None:
        m = array([1] * n)
...
def draw_image(X, res, r=None, board_size=None):
    T, n = shape(X)[0:2]
...
def bounce_vec(res, n=2, T=128, r=None, m=None, board_size=None, ball_move_size=None):
    valid = False
    while not valid:
        # x, y, init_a, key_frame, col_frame = bounce_n(T, n, r, m)
        x, y, init_a = bounce_n(T, n, r, m, ball_move_size)
        x[:, :, 1] += board_size[1]
        x[:, :, 0] += board_size[0]
        valid = True
        ...
        V, board_mask = draw_image(x, res, r, board_size)
...
def gen_data(N, num_objs, name, show_video=False):
    ...
    for i in tqdm(range(N)):
        ...
        dat[i], dat_y[i], init_a[i], dat_mask[i] = bounce_vec(res=res, n=num_objs, T=T, r=r, board_size=board_size, ball_move_size=ball_move_size)
        dat_r[i] = r.copy()
